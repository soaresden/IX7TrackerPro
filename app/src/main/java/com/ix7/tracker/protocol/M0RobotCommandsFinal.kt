 ...
 */
object M0RobotCommandsFinalFinal {
     ...
     */
    val MODE_PEDESTRIAN = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0x49, 0x37, 0x34, 0x6C, 0xCB.toByte()
    )

    val MODE_D = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0x48, 0x36, 0x34, 0x6E, 0xCB.toByte()
    )

    val MODE_S = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0x4A, 0x36, 0x34, 0x6C, 0xCB.toByte()
    )

    val MODE_S_PLUS = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0x4A, 0x35, 0x34, 0x6D, 0xCB.toByte()
    )

    // ========== COMMANDES DÉCOUVERTES (haute probabilité) ==========

    /**
     * NÉONS - Basé sur le pattern 0xC5 trouvé dans les logs
     * Position CMD = 0xC5, pattern similaire aux phares (C6)
     */
    val NEON_ON = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0xC5.toByte(), 0x35, 0x34, 0xD0.toByte(), 0xCA.toByte()
    )

    val NEON_OFF = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0xC5.toByte(), 0x34, 0x34, 0xD3.toByte(), 0xCA.toByte()
    )

    /**
     * RÉGULATEUR DE VITESSE - Pattern 0x48
     * Trouvé dans les logs période 18:13-18:14
     */
    val CRUISE_ON = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
        0x48, 0x35, 0x34, 0x6F, 0xCB.toByte()
    )

    val CRUISE_OFF = byteArrayOf(
        0x61, 0x9E.toByte(), 0x30, 0x14, 0x37,
